...
function scheduleDraftSave() {
	...
	const title = titleInputRef?.value.trim() ?? document.getElementById('title')?.value.trim();
	if (!title) return;
...
async function saveDraft() {
	...
	const title = titleInputRef?.value.trim() ?? document.getElementById('title')?.value.trim();
	const body = bodyInputRef?.value.trim() ?? document.getElementById('body')?.value.trim();
	const categoryIds = getSelectedCategoryIds(getCategoryHost());
	const pendingImageFile =
		imageInputRef?.files?.[0] ?? document.getElementById('image')?.files?.[0];
...
async function restoreDraftIfExists() {
	try {
		...
		const restore = await uiConfirm('Draft found. Do you want to restore it?', {
			type: 'warn',
		...
		setSelectedCategoryIds(document.getElementById('categoryCheckboxes'), data.category_ids || []);
	...
	form.addEventListener('submit', async (e) => {
		...
		try {
			...
			if (!res.ok) {
				...
				const payload = await res.json().catch(() => null);
				uiNotify(payload?.error?.message || 'Failed to publish post.', {
					type: 'danger',
				});
				return;
			}
